                  ...
                  uint8_t *out_ciphertext)
{
    ...
    if (ok && EVP_EncryptInit_ex(ctx, EVP_aes_256_gcm(), nullptr, nullptr, nullptr) != 1)
        ok = false;
    if (ok && EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN, static_cast<int>(NONCE_LEN), nullptr) != 1)
        ok = false;
    if (ok && EVP_EncryptInit_ex(ctx, nullptr, nullptr, key, out_nonce) != 1)
        ok = false;
    ...
    if (ok && EVP_EncryptFinal_ex(ctx, out_ciphertext + out_len, &len) != 1)
        ok = false;
    if (ok)
        out_len += len;

    if (ok && EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, static_cast<int>(TAG_LEN), out_tag) != 1)
        ok = false;
                  ...
                  uint8_t *out_plaintext)
{
    ...
    if (ok && EVP_DecryptInit_ex(ctx, EVP_aes_256_gcm(), nullptr, nullptr, nullptr) != 1)
        ok = false;
    if (ok && EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN, static_cast<int>(NONCE_LEN), nullptr) != 1)
        ok = false;
    if (ok && EVP_DecryptInit_ex(ctx, nullptr, nullptr, key, nonce) != 1)
        ok = false;
    ...
    if (ok && EVP_DecryptFinal_ex(ctx, out_plaintext + out_len, &len) <= 0)
        ok = false;
