...
int main(int argc, char** argv) {
    ...
    enum { OPT_MODE = 1, OPT_SERVER, OPT_PORT, OPT_TUN_IP, OPT_KEY };
    ...
    while ((opt = getopt_long(argc, argv, "", long_opts, nullptr)) != -1) {
        switch (opt) {
            ...
            case OPT_SERVER: server_ip = optarg; break;
            case OPT_PORT:   port = static_cast<uint16_t>(std::atoi(optarg)); break;
            case OPT_TUN_IP: tun_ip = optarg; break;
            case OPT_KEY:    key_path = optarg; break;
            default:
                usage(argv[0]);
                return 1;
        }
